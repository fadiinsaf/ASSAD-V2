<?php 
    ...
    if(!isset($_SESSION["user_id"]) || $_SESSION["role"] !== "visiter"){
        echo "Current role is: " . ($_SESSION["role"] ?? "NOT SET");
        header("Location: ../index.html");
        exit();
    }
    require_once __DIR__ . "/../database/database.php";
    $stmt = $db->query("SELECT * FROM animals", PDO::FETCH_ASSOC);
...
<body class="bg-background-dark text-white overflow-x-hidden selection:bg-primary selection:text-background-dark">
    <div class="relative flex h-auto min-h-screen w-full flex-col">
        ...
        <main class="flex flex-col w-full items-center">
            <section class="w-full max-w-7xl px-4 md:px-10 lg:px-20 py-8">
                <div class="w-full relative rounded-3xl overflow-hidden min-h-[600px] flex flex-col justify-end p-6 md:p-14 border border-surface-border shadow-2xl group"
                    data-alt="Close up portrait of a majestic male lion with a full mane looking into the distance"
                    style='background-image: linear-gradient(180deg, rgba(16, 34, 24, 0) 0%, rgba(16, 34, 24, 0.4) 50%, rgba(16, 34, 24, 0.95) 100%), url("https://lh3.googleusercontent.com/aida-public/AB6AXuAERPy8vrNeJzxeIK4ZdHRxsJOgalHQoNV5erONeg_QnI3Yjb7XBsz_ZfoKS5LHaRJolimpybS4hOXTrjJWF1TxJNWm69lRevbo48G9BF18ekbE8PhUbYQhQp1mi-rLiVsmkJspQU7CH3e5nEYoyl7SoTCqOhZ7eL8chpsl1aR5pMHOAVJeUcM3HCJAs0ANvrvUOUXxgx35YXXHnbLAALB5MZa337tmI4oS_G83vtrpjFE2B8-ethUv9-ixi8qMe9lctHBj2OcUQCSY"); background-size: cover; background-position: center 20%;'>
                    <div class="relative z-10 max-w-3xl flex flex-col gap-6 animate-fade-in">
                        <h1
                            class="text-5xl md:text-7xl font-extrabold text-white leading-[1.1] tracking-tight font-display drop-shadow-sm">
                            Roar with Pride <br />
                            <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary to-sand">Meet
                                Asaad</span>
                        </h1>
                        <p class="text-lg md:text-xl text-gray-200 max-w-xl font-light leading-relaxed">
                            Discover the majestic Atlas Lion and African wildlife from your home. Join our rangers for a
                            live virtual safari today.
                        </p>
                        <div class="flex flex-wrap gap-4 mt-4">
                            <a href="animal_list.php"
                                class="flex items-center justify-center rounded-full h-14 px-8 bg-primary text-background-dark font-bold hover:scale-105 transition-transform font-display text-base">
                                Explore Animals
                            </a>
            ...
            <section class="w-full max-w-7xl px-4 md:px-10 lg:px-20 py-12">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <h2 class="text-3xl md:text-4xl font-bold text-white font-display mb-2">Stars Of Assad Zoo
                        </h2>
                        <p class="text-gray-400 font-light">Meet our most popular Animals</p>
                    </div>
                    <a class="text-primary font-bold hover:text-sand transition-colors flex items-center text-sm uppercase tracking-wider"
                        href="animal_list.php">
                        View All <span class="material-symbols-outlined text-lg ml-1">arrow_forward</span>
                    </a>
                </div>

                <div class="flex overflow-x-auto no-scrollbar gap-6 pb-8 snap-x snap-mandatory">
                    ...
                    <?php
                        ...
                        if(count($animals) > 0){
                            foreach($animals as $animal){
                                echo "
                                    <div class='flex-none w-[300px] md:w-[340px] snap-center group cursor-pointer'>
                                        <div
                                            class='relative h-[440px] w-full rounded-3xl overflow-hidden mb-4 border border-surface-border bg-surface-dark shadow-lg'>
                                            <div class='absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-110'
                                                data-alt='animal image' style=\"background-image: url('/../assets/images/{$animal['image']}');\">

                                            </div>
                                            <div
                                                class='absolute inset-0 bg-gradient-to-t from-background-dark via-background-dark/50 to-transparent opacity-90'>
                                            </div>
                                            <div class='absolute bottom-0 left-0 p-8 w-full'>
                                                <h3
                                                    class='text-2xl font-bold text-white font-display mb-2 group-hover:text-primary transition-colors'>
                                                    {$animal['name']}</h3>
                                                <p class='text-gray-300 text-sm leading-relaxed line-clamp-2'>{$animal['short_description']}</p>
                                            </div>
                                        </div>
                                    </div>
                                ";
                            ...
                            echo "<h3 class='text-2xl font-bold font-display'>No Animals For Display</h3>";
                        }
